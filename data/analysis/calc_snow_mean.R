...
setwd(dir_pred)
pred_files <- list.files(dir_pred, pattern="*.tif")
mean_dir <- "E:/Masterarbeit/Data/validation_stacks/snow_mean/"
...
for(y in years){
  filtered <- list.files(dir_pred, pattern = paste0("_", y))
...
for(m in months){
  filtered <- list.files(dir_pred, pattern = paste0(".*", m, ".{6}\\.tif$"))
  stack_filt <- stack(filtered)
