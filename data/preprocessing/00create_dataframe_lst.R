...
terra_list <- list.files("C:/Users/andre/OneDrive/Dokumente/Masterarbeit/Data/lst", pattern = "terra.tif$", full.names = TRUE)
aqua_list <- list.files("C:/Users/andre/OneDrive/Dokumente/Masterarbeit/Data/lst", pattern = "aqua.tif$", full.names = TRUE)
...
names(aqua)[names(aqua) == "value"] <- "lst"
aqua$sat <- rep('aqua',nrow(aqua))

terra <- melt(terra_train)
names(terra)[names(terra) == "value"] <- "lst"
terra$sat <- rep('terra',nrow(terra))

write.csv(terra,"dataframes/lst_terra_500.csv", row.names = FALSE)
write.csv(aqua,"dataframes/lst_aqua_500.csv", row.names = FALSE)
